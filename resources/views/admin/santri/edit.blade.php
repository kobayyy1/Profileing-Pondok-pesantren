...
<head>
    ...
    <title>Edit Data Santri – Control Panel</title>
...
<body class="bg-slate-50 text-gray-800 font-sans flex min-h-screen">
    ...
    <main class="flex-1 flex flex-col min-w-0">
        <header class="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center shadow-sm">
            <h1 class="text-sm font-bold text-blue-950 uppercase tracking-wider">Edit Data Santri</h1>
            <a href="{{ route('admin.santri.index') }}" class="text-xs font-bold text-blue-900 hover:underline"><i
        ...
        <div class="p-6 md:p-8 max-w-3xl w-full mx-auto space-y-6 overflow-y-auto">
            ...
            <form action="{{ route('admin.santri.update', $santri->id) }}" method="POST" class="space-y-6">
                ...
                <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="nisn" value="{{ old('nisn', $santri->nisn) }}" required
                                class="w-full border border-gray-300 rounded-lg px-3.5 py-2 text-xs focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600">
                        </div>
                        <div>
                            ...
                            <select name="kelas" required
                                ...
                                <option value="7-A" {{ old('kelas', $santri->kelas) == '7-A' ? 'selected' : '' }}>
                                    Kelas 7-A (MTs)</option>
                                <option value="7-B" {{ old('kelas', $santri->kelas) == '7-B' ? 'selected' : '' }}>
                                    Kelas 7-B (MTs)</option>
                                <option value="10-A" {{ old('kelas', $santri->kelas) == '10-A' ? 'selected' : '' }}>
                                    Kelas 10-A (MA)</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
                    ...
                    <div>
                        <label class="block text-[11px] font-bold text-gray-700 uppercase mb-1">Nama Lengkap Orang Tua /
                            Wali</label>
                        <input type="text" name="nama_ortu" value="{{ old('nama_ortu', $santri->orangTua->name) }}"
                            required
                            class="w-full border border-gray-300 rounded-lg px-3.5 py-2 text-xs focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="email" name="email_ortu"
                            value="{{ old('email_ortu', $santri->orangTua->email) }}" required
                            class="w-full border border-gray-300 rounded-lg px-3.5 py-2 text-xs focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600">
                    </div>
                </div>
                ...
                <button type="submit"
                    ...
                    <i class="fas fa-check-double mr-1"></i> Perbarui & Simpan Perubahan
                </button>
            </form>
